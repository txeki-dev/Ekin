...
## Corpus Check
- 80 files · ~97,987 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1218 nodes · 2120 edges · 127 communities (84 shown, 43 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 108 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `84a406c3`
...
Cohesion: 0.10
Nodes (32): QDialog, SettingsDialog, _card_with_timer(), _make_task(), Pruebas de humo (smoke tests) headless para widgets de Qt: construcción y unas…, Regresión: el texto describía el comportamiento antiguo de Ctrl+N (siempre la…, Regresión de la fuga de memoria: el diálogo real (parentado a MainWindow/…, El nuevo self.finished.connect(self.deleteLater) no debe romper el patrón ya… (+24 more)
...
Cohesion: 0.12
Nodes (14): Vista de calendario mensual para Ekin: muestra las tareas por su fecha de…, Diálogo de búsqueda global de tareas. Filtra por texto (título/descripción),…, Pantalla de Ajustes de la aplicación: tema, notificaciones y persistencia.…, Diálogo "Atajos de teclado" (Ctrl+/): referencia estática de todos los atajos…, color_swatch_css(), hex_to_rgb(), Aplica el tema oscuro estándar (fondo/borde/item/selección) a un QMenu., CSS para un botón de muestra de color (color + borde + esquinas redondeadas).… (+6 more)
...
Cohesion: 0.24
Nodes (3): MainWindow, Reescribe cada feed .ics con auto-sync configurado (el global de todos los…, QMainWindow
...
Cohesion: 0.25
Nodes (6): Alterna la barra lateral y actualiza el icono: ◀ (plegar) / ▶ (desplegar)., color_icon(), Genera un pequeño icono cuadrado del color indicado (para combos y listas)., Cadenas de la interfaz, centralizadas para facilitar una futura traducción.…, make_glyph_icon(), Dibuja un icono (triángulo o lápiz) como QPixmap, sin depender de fuentes. Los…
...
### Community 121 - "_ClickOutsideFilter"
Cohesion: 0.33
Nodes (4): _ClickOutsideFilter, Filtro de eventos que detecta clics fuera del diálogo dentro de la ventana…, Abre el diálogo de forma síncrona sin bloquear la ventana padre, permitiendo…, QObject
...
Cohesion: 0.50
Nodes (4): fit_html_images(), Ajusta o añade el atributo width a las etiquetas <img> para que nunca desborden…, fit_html_images ajusta o añade width para evitar desbordamiento horizontal., test_log_entry_widget_fit_html_images_constrains_width()

### Community 124 - "ClickableTagPill"
Cohesion: 0.50
Nodes (3): ClickableTagPill, QFrame, Pastilla de etiqueta cuyo cuerpo emite `clicked` (para editar el valor). El…

### Community 125 - "main"
Cohesion: 0.67
Nodes (3): apply_win32_icon(), main(), Fuerza los iconos nativos de Win32 (WM_SETICON) directamente en el HWND de…
...
- **Why does `t()` connect `t` to `CalendarViewWidget`, `TaskDetailDialog`, `test_widgets_headless.py`, `.init_ui`, `._build_column_widget`, `SidebarWidget`, `main.py`, `.load_board`, `QPushButton`, `.load_task_data`, `task_detail_dialog.py`, `TaskCard`, `LogEntryWidget`, `.__init__`, `BoardButton`, `RichTextToolbar`, `BoardEditDialog`, `NotificationsPopup`, `ImagePreviewDialog`, `.reload_boards`, `.add_task`, `.render_tags`, `.notify_due_today`, `.delete_board`, `.check_for_updates`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `TaskDetailDialog` connect `TaskDetailDialog` to `LogEntryWidget`, `MarkdownTextEdit`, `RichTextToolbar`, `test_widgets_headless.py`, `.init_ui`, `.render_tags`, `._build_column_widget`, `.load_task_data`, `t`, `main.py`, `._open_task_detail`, `.reload_logs`, `_ClickOutsideFilter`, `ClickableTagPill`, `task_detail_dialog.py`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `BoardViewWidget` connect `BoardViewWidget` to `TaskCard`, `.__init__`, `.add_task`, `.handle_task_drop`, `test_timer_board_view.py`, `._build_column_widget`, `t`, `SidebarWidget`, `main.py`, `.load_board`, `test_hover_expand.py`, `QPushButton`, `MainWindow`, `task_detail_dialog.py`?**
