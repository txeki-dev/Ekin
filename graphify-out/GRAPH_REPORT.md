...
## Summary
- 1138 nodes · 2015 edges · 121 communities (77 shown, 44 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 96 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `76b5ac28`
...
3. `BoardViewWidget` - 61 edges
4. `TaskDetailDialog` - 60 edges
5. `SidebarWidget` - 32 edges
6. `MainWindow` - 32 edges
7. `MarkdownTextEdit` - 28 edges
8. `ColumnWidget` - 27 edges
9. `TagManagerDialog` - 24 edges
10. `TaskCard` - 23 edges
...
Cohesion: 0.07
Nodes (22): MarkdownTextEdit, QTextEdit con atajos tipo Markdown para crear listas al vuelo. - `* `, `- `, `+…, Al pegar: las imágenes se insertan como imagen; una tabla (de Excel/Sheets/Word…, Si el texto plano pegado tiene pinta de tabla (varias líneas con tabuladores,…, Inserta una tabla `rows`x`cols` en la posición del cursor, con el estilo del…, Codifica un QImage como data URI PNG en base64., Embebe un QImage como data URI base64 (queda guardado dentro del HTML). Se…, Elimina el marcador escrito y convierte la línea actual en una lista. (+14 more)

### Community 3 - "TaskDetailDialog"
Cohesion: 0.09
Nodes (12): QDialog, Habilita/inhabilita fecha y hora según los checks., Asigna (o reemplaza) el valor de una etiqueta permanente, garantizando un único…, Retira una etiqueta de la tarea (localmente) y re-renderiza., Asigna una etiqueta permanente (categoría) con uno de sus valores a la tarea., Guarda el título, descripción, etiquetas y fecha de vencimiento., Inicia el temporizador, o lo reinicia a ahora si ya estaba en marcha. Acción…, Detiene y borra el temporizador: deja de contar y quita la insignia de la… (+4 more)
...
Cohesion: 0.15
Nodes (23): _card_with_timer(), _make_task(), Pruebas de humo (smoke tests) headless para widgets de Qt: construcción y unas…, Regresión de la fuga de memoria: el diálogo real (parentado a MainWindow/…, El nuevo self.finished.connect(self.deleteLater) no debe romper el patrón ya…, test_add_link_with_local_path_renders_with_attachment_icon(), test_add_link_with_web_url_renders_with_link_icon(), test_browse_local_file_does_not_overwrite_existing_label() (+15 more)
...
### Community 9 - "QLabel"
Cohesion: 0.16
Nodes (7): QLabel, QPushButton, QDialog, ShortcutsDialog, Barra con reloj (fecha/hora) en su propia fila arriba, y accesos rápidos…, test_shortcuts_dialog_constructs_with_both_sections(), Limpia y dibuja las etiquetas actuales y la fecha de vencimiento.
...
### Community 11 - "t"
Cohesion: 0.14
Nodes (10): Sustituye el contenido por un editor en línea con Guardar/Cancelar., Carga los datos iniciales de la tarea y sus logs desde la base de datos., Dibuja las etiquetas asignadas como pastillas. Clic en la pastilla = editar el…, Devuelve el id de la etiqueta permanente «Prioridad», asegurando que existan…, Rellena el selector rápido de Prioridad con los valores actuales del catálogo…, Ajusta la selección del combo de Prioridad a lo que haya en current_tags, sin…, Rellena el selector de Tablero vinculado con el resto de tableros (excluyendo…, Actualiza el botón y la etiqueta de tiempo transcurrido según… (+2 more)
...
Cohesion: 0.12
Nodes (8): Registra una acción deshacer/rehacer para un borrado (restaurar desde snapshot)., Construye un ColumnWidget completo (señales conectadas y, si está desplegada,…, Reconstruye el ColumnWidget de UNA sola columna (datos/tareas frescos de la BD)…, Confirma y borra una columna., Expansión temporal (por hover durante un arrastre) de una columna plegada:…, Repliega (BD + widget) la columna actualmente expandida por hover, si la hay.…, Conectado a TaskCard.drag_ended: se ejecuta al terminar cualquier arrastre de…, Abre el diálogo de detalle/chat de una tarea.
...
### Community 17 - "styles.py"
Cohesion: 0.12
Nodes (13): Diálogo de búsqueda global de tareas. Filtra por texto (título/descripción),…, _swatch_icon(), Pantalla de Ajustes de la aplicación: tema, notificaciones y persistencia.…, Diálogo "Atajos de teclado" (Ctrl+/): referencia estática de todos los atajos…, build_qss(), color_swatch_css(), Aplica el tema oscuro estándar (fondo/borde/item/selección) a un QMenu., CSS para un botón de muestra de color (color + borde + esquinas redondeadas).… (+5 more)
...
Cohesion: 0.31
Nodes (12): BoardViewWidget, Refresca la insignia de tiempo transcurrido en todas las tarjetas con un…, _make_board(), test_add_task_sets_last_active_column(), test_column_background_click_sets_last_active_column(), test_column_widget_mouse_press_emits_column_activated(), test_quick_add_task_falls_back_to_first_column_when_nothing_active(), test_quick_add_task_falls_back_when_last_active_column_belongs_to_another_board() (+4 more)

### Community 25 - "MainWindow"
Cohesion: 0.20
Nodes (4): MainWindow, Reescribe cada feed .ics con auto-sync configurado (el global de todos los…, Muestra u oculta la barra lateral., QMainWindow
...
### Community 28 - "board_view.py"
Cohesion: 0.22
Nodes (6): BoardSelectionDialog, Diálogo para seleccionar un tablero de destino para mover o copiar una columna., hex_to_rgb(), Convierte un color hexadecimal en formato string a una tupla RGB (r, g, b)., Pila simple de deshacer/rehacer para acciones destructivas (borrar…, UndoAction
...
### Community 30 - "task_detail_dialog.py"
Cohesion: 0.24
Nodes (6): ClickableTagPill, color_icon(), QFrame, Pastilla de etiqueta cuyo cuerpo emite `clicked` (para editar el valor). El…, Genera un pequeño icono cuadrado del color indicado (para combos y listas)., Cadenas de la interfaz, centralizadas para facilitar una futura traducción.…

### Community 31 - "TagPickerDialog"
Cohesion: 0.23
Nodes (5): QDialog, Selecciona una etiqueta del catálogo para una tarea. - Modo asignar…, Devuelve (tag_value_id | None, is_none). is_none indica que se eligió «Ninguno»., TagPickerDialog, Edita el valor de una etiqueta ya asignada: cambiarlo o poner «Ninguno»…

### Community 32 - "TaskCard"
Cohesion: 0.17
Nodes (6): QFrame, Aplica dinámicamente el estilo a la tarjeta basándose en el color de fondo del…, Dibuja (o esconde) la pastilla clicable hacia el tablero enlazado, si lo hay., Umbral (en horas) a partir del cual la insignia del temporizador se resalta en…, Dibuja (o esconde) la insignia de tiempo transcurrido del temporizador, en rojo…, TaskCard
...
Cohesion: 0.20
Nodes (8): LogEntryWidget, QFrame, Una entrada del diario/chat, con botones (pintados) de editar y eliminar y…, Maneja los enlaces clicados dentro de una entrada ya enviada. Hoy el único tipo…, Regresión: setTextInteractionFlags(Qt.TextSelectableByMouse) A SOLAS anulaba…, test_log_entry_widget_plain_text_keeps_default_cursor(), test_log_entry_widget_real_click_on_posted_image_opens_preview(), test_log_entry_widget_routes_image_link_to_preview()
...
Cohesion: 0.20
Nodes (3): BoardButton, QFrame, Widget personalizado para representar un botón de tablero en la barra lateral.
...
### Community 38 - "log_entry.py"
Cohesion: 0.25
Nodes (8): pixmap_from_data_uri(), Decodifica 'data:image/xxx;base64,....' a un QPixmap. Devuelve un QPixmap nulo…, Abre ImagePreviewDialog para el data URI dado. No-op si no decodifica a una…, show_image_preview(), test_pixmap_from_data_uri_decodes_valid_png(), test_pixmap_from_data_uri_returns_null_for_garbage(), test_show_image_preview_noop_for_invalid_uri(), test_show_image_preview_opens_dialog_for_valid_image()

### Community 39 - "RichTextToolbar"
Cohesion: 0.18
Nodes (4): QWidget, Barra de formato básica (negrita, cursiva, tachado, viñetas, tablas) para un…, Pide filas y columnas y crea una tabla vacía en la posición del cursor., RichTextToolbar

### Community 40 - "BoardColumnsArea"
Cohesion: 0.27
...
### Community 42 - "NotificationsPopup"
Cohesion: 0.24
...
### Community 54 - "._init_collapsed_ui"
Cohesion: 0.22
Nodes (4): Etiqueta con el texto girado 90° (nombre de una columna plegada)., Pequeño botón cuadrado con un icono PINTADO (left/right/pencil) a juego con el…, Columna plegada: tira estrecha con botón de desplegar, contador y nombre…, VerticalLabel

### Community 55 - "ColumnEditDialog"
Cohesion: 0.32
Nodes (3): ColumnEditDialog, QDialog, Diálogo para crear o editar una columna (nombre y color).
...
### Community 58 - "SearchDialog"
Cohesion: 0.36
Nodes (4): QDialog, Reejecuta la búsqueda con los filtros actuales y repinta la lista., Búsqueda global de tareas con filtros por tablero, etiqueta y vencimiento., SearchDialog

### Community 59 - "SettingsDialog"
Cohesion: 0.25
...
### Community 60 - "_collapsed_column_widget"
Cohesion: 0.36
...
### Community 70 - "make_glyph_icon"
Cohesion: 0.50
Nodes (3): Alterna la barra lateral y actualiza el icono: ◀ (plegar) / ▶ (desplegar)., make_glyph_icon(), Dibuja un icono (triángulo o lápiz) como QPixmap, sin depender de fuentes. Los…
...
## Knowledge Gaps
- **91 isolated node(s):** `ekin-kanban`, `create_log() Atomicity Fix (Fix 6)`, `Dead app.setStyleSheet() Removal (Fix 8)`, `Stale Shortcuts Help Text Fix (Fix 3)`, `timer_alert_hours N+1 Read Fix (Fix 7)` (+86 more)
...
- **Why does `t()` connect `t` to `CalendarViewWidget`, `TaskDetailDialog`, `test_widgets_headless.py`, `QLabel`, `._build_column_widget`, `TagManagerDialog`, `SidebarWidget`, `styles.py`, `.load_board`, `ColumnWidget`, `board_view.py`, `task_detail_dialog.py`, `TagPickerDialog`, `LogEntryWidget`, `.__init__`, `BoardButton`, `log_entry.py`, `RichTextToolbar`, `BoardColumnsArea`, `BoardEditDialog`, `NotificationsPopup`, `.reload_boards`, `._init_collapsed_ui`, `ColumnEditDialog`, `SearchDialog`, `SettingsDialog`, `._init_expanded_ui`, `.add_task`, `.notify_due_today`, `.delete_board`, `.check_for_updates`, `test_shortcuts_item_new_task_describes_last_active_column_behavior`?**
  _High betweenness centrality (0.163) - this node is a cross-community bridge._
- **Why does `BoardViewWidget` connect `BoardViewWidget` to `TaskCard`, `.__init__`, `make_glyph_icon`, `BoardColumnsArea`, `.add_task`, `.handle_task_drop`, `test_timer_board_view.py`, `._build_column_widget`, `SidebarWidget`, `.load_board`, `test_hover_expand.py`, `ColumnWidget`, `board_view.py`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `TaskDetailDialog` connect `TaskDetailDialog` to `LogEntryWidget`, `MarkdownTextEdit`, `RichTextToolbar`, `test_widgets_headless.py`, `t`, `.open_tag_manager`, `._build_column_widget`, `TagManagerDialog`, `._open_task_detail`, `.reload_logs`, `board_view.py`, `task_detail_dialog.py`, `TagPickerDialog`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Are the 3 inferred relationships involving `BoardViewWidget` (e.g. with `UndoAction` and `ColumnWidget`) actually correct?**
  _`BoardViewWidget` has 3 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 30 inferred relationships involving `QLabel` (e.g. with `.init_ui()` and `.init_ui()`) actually correct?**
  _`QLabel` has 30 INFERRED edges - model-reasoned connections that need verification._
- **What connects `ekin-kanban`, `create_log() Atomicity Fix (Fix 6)`, `Dead app.setStyleSheet() Removal (Fix 8)` to the rest of the system?**
