...
- **Rich Notes & Code Blocks**: The description and diary support **bold** (Ctrl+B), *italic* (Ctrl+K), ~~strikethrough~~ (Ctrl+Shift+X), **blockquotes / citations** (Ctrl+Shift+Q or `> `), **full paragraph alignment** (Left Ctrl+L, Center Ctrl+E, Right Ctrl+R, Justify Ctrl+J), **case transformations** (UPPERCASE Ctrl+Shift+U, lowercase Ctrl+Shift+L, toggle Shift+F3), **syntax-highlighted code blocks** (type ` ``` ` or click `</>`), **horizontal separators** (`---` or `―`), **text color customization**, nested bullet lists (Tab to indent), and **clickable hyperlinks** (auto-linkified plain URLs or via `🔗`). External text can be pasted with original styling (Ctrl+V) or as plain text (Ctrl+Shift+V). Supports **pasting images** with context menu resizing (25%, 50%, 75%, 100%, custom px) and zoom preview, **Word-style tables**, and **local file/folder path detection** which automatically creates links in the task attachments panel. Includes a last-edited timestamp visible in the Notes header.
...
- **Priority**: A quick **Priority** selector in the task detail (Low/Medium/High by default, customizable) shows up as its own pill row on the board card.
- **Task Timer**: Click **Start** in the task detail to record when you started it; a live "time elapsed" counter shows next to it, and the same badge appears right on the board card — turning terracotta once it crosses a configurable threshold in Settings, so stale tasks stand out without opening each one.
- **Board Links**: Point a task at a *different* board — e.g. a summary task in "Tareas" that links to the dedicated "SW X" board where its detailed work lives. Pick the target from **Linked board** in the task detail; the card then shows a clickable pill that jumps straight there.
...
- **Shared Boards & Cloud Linking (Google Drive, Dropbox, OneDrive, LAN)**: Link any board to an `.ekboard` file or open existing shared boards from collaborators directly from the header or sidebar. Features an offline-first **two-way differential merge engine** with automated pre-merge snapshots, non-blocking asynchronous background sync, reactive file watching, and **no-data-loss conflict resolution** (superseded concurrent edits are automatically archived into the task's personal diary/chat).
- **Bulk Task Creation**: Quickly define and import multiple tasks at once through the dedicated Bulk Add modal next to Cloud Sync. Supports column assignment and copy-pasting directly from spreadsheets (Excel, Google Sheets).
- **Multi-Card Selection (`Ctrl + Click`) & Autonomous Local AI Spec Generator (Vía B)**: Select multiple task cards across columns with `Ctrl + Click` to reveal the action dock. Synthesizes technical architecture specifications (SPEC) optimized for autonomous AI coding agents (Google Antigravity, Claude Code, Cursor, Windsurf), User Stories with Gherkin acceptance criteria, or QA test matrices. Powered by a self-managed local neural runner (Qwen 2.5 Coder 1.5B ~980 MB), dynamic Ollama model selector with live refresh (`🔄`), and instant offline structural fallback.
- **Internationalization (i18n)**: Full bilingual support (**English** by default, **Español**) with live hot-reloading across the entire UI, boards, sidebar, and tray menu, switchable instantly in Settings.
- **Themes & Settings**: A settings screen with a **Light / Dark theme** toggle (light "Warm Shell" is the default), language selector, notification prefs, a configurable task-timer alert threshold, and a remembered window size.
