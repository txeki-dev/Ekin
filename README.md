...
- **Rich Notes & Code Blocks**: The description and diary support **bold** (Ctrl+B), *italic* (Ctrl+K), ~~strikethrough~~ (Ctrl+Shift+X), **full paragraph alignment** (Left Ctrl+L, Center Ctrl+E, Right Ctrl+R, Justify Ctrl+J), **case transformations** (UPPERCASE Ctrl+Shift+U, lowercase Ctrl+Shift+L, toggle Shift+F3), **syntax-highlighted code blocks** (type ` ``` ` or click `</>`), **horizontal separators** (`---` or `―`), **text color customization**, nested bullet lists (Tab to indent), and **clickable hyperlinks** (auto-linkified plain URLs or via `🔗`). External text pastes as **plain text**, and you can **paste images** — click one to open it larger — and **tables** (from Excel/Sheets/Word, or tab-separated text) straight in, or insert Microsoft Word-style styled tables with vertically and horizontally centered cells.
