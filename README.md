...
- **Shared Boards & OneDrive Synchronization**: Link any board to an `.ekboard` file inside OneDrive, Google Drive, Nextcloud, or a local LAN share. Features an offline-first **two-way differential merge engine** with automated pre-merge snapshots, reactive file watching, and **no-data-loss conflict resolution** (superseded concurrent edits are automatically archived into the task's personal diary/chat).
- **Multi-Card Selection (`Ctrl + Click`) & Autonomous Local AI Spec Generator (Vía B)**: Select multiple task cards across columns with `Ctrl + Click` to reveal the action dock. Synthesizes technical architecture specifications (SPEC) optimized for autonomous AI coding agents (Google Antigravity, Claude Code, Cursor, Windsurf), User Stories with Gherkin acceptance criteria, or QA test matrices. Powered by a self-managed, zero-setup local neural runner (Qwen 2.5 Coder 1.5B ~980 MB) with instant offline structural fallback.
